...
for ym in all_months:
    ...
    # 仅保留股价 >= 3 且不在剔除名单中的股票
    filtered = month_df[
        (month_df['open_price'] >= 3) & (~month_df['ticker'].isin(excluded_tickers))
        ].copy()
    ...
    # 选择价格最低的 50 支股票
    selected = filtered.nsmallest(25, 'open_price')
    ...
    # 找出 top5 盈利股票，下个月剔除
    top5 = selected.sort_values('return', ascending=False).head(0)
